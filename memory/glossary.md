...
| Second-digit analysis | Comparing the observed frequency of the second significant digit (0–9) against the Benford-expected frequency; planned for M3/v1.0 |
| Combined analysis | An M3/v1.0 view that computes first- and second-digit distributions from one preprocessing snapshot and displays both independent results together; not a joint 10–99 first-two-digit distribution |
...
| Drill-down | Clicking a digit in a results chart to view the original rows whose corresponding first or second significant digit matches |
