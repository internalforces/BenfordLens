...
| TD-011 | Selected Actions, full-SHA enforcement, dependency alerts/updates, and active no-bypass `main`/release-tag rulesets are enforced; public-only scanning/reporting remains unavailable while the repository is private | Secret scanning, push protection, private vulnerability reporting, and the first CodeQL result cannot be proven before visibility changes | Finish TASK-042 immediately after launch by enabling and verifying every public-only security feature and rechecking the rulesets |
