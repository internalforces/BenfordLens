...
| 영역 | 결과 |
|------|------|
| 자동화 품질 | 현재 기준선에서 Ruff·형식 검사·22개 소스 파일 mypy·259개 테스트가 모두 통과 |
...
현재 검증 결과는 259개 테스트 통과입니다. 테스트 범위, 성능 측정 방식, 패키징 검사, 명확한
...
- [로드맵](roadmap.md) — 하나로 정리한 필수 후속 배포 마일스톤

위 문서는 현재 유지하는 공개 탐색 경로입니다. 필요한 과거 구현 세부사항은 Git 이력에서
확인할 수 있습니다.

## 커뮤니티와 고지

- [기여 안내](CONTRIBUTING.md) — 개발 환경, 프로젝트 경계, Pull Request
- [지원 범위](SUPPORT.md) — 사용 도움말, 지원 대상, 안전한 합성 데이터 재현
- [보안 정책](SECURITY.md) — 비공개 취약점 제보와 지원 버전
- [행동 강령](CODE_OF_CONDUCT.md) — 존중하는 참여와 비공개 제보
- [제3자 소프트웨어 고지](THIRD_PARTY_NOTICES.md) — 정확한 런타임 목록, 라이선스 전문,
  소스·귀속 정보와 Qt 재링크 안내
...
Benford Lens는 [MIT License](LICENSE)로 제공됩니다. 제3자 구성 요소에는
[제3자 소프트웨어 고지](THIRD_PARTY_NOTICES.md)에 기록된 각 라이선스 조건이 계속
적용됩니다.
